...
class BertNerTagger(pl.LightningModule):
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        vocab_path = 'output/vocab_files/vocab.txt'
        print("using AutoTokenizer as the tokenizer ")
        model_type = self.bert_dir
        output_path = 'output/vocab_files/'
        # tokenizer = AutoTokenizer.from_pretrained(model_type)
        # vocab_files = download_vocab_files_for_tokenizer(tokenizer, model_type, output_path)
        # BertTok = BertWordPieceTokenizer(vocab_files.get('vocab_file'), vocab_files.get('merges_file'),unk_token='[UNK]')
        BertTok = BertWordPieceTokenizer(vocab_path, unk_token='[UNK]')

        #print("PRINTING TOKENIZER !!!!! :", BertTok)
        dataset = BERTNERDataset(self.args, json_path=json_path,
                                # tokenizer=tokenizer,
                                 tokenizer=BertTok,
                                 # tokenizer=BertWordPieceTokenizer(vocab_file=vocab_path),
        ...
        if prefix=='train':
            ...
            train_split_index = int(np.floor(1 * size))
        ...
        dataloader = DataLoader(
            dataset=dataset,
            batch_size=4 if prefix == 'train' else self.args.batch_size,
            num_workers=16,
            shuffle=False,
            sampler = train_sampler if prefix == "train" else None,
            # shuffle=False,
            drop_last=False,
        ...
        if prefix=='train':
            print("Length of Train Data before sampling:",size)
            print("length of Sampled Train Data:",len(dataloader))
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath=args.default_root_dir, #Changed filepath to dirpath
        save_top_k=1,
        verbose=True,
        monitor="span_f1",
        period=-1,
    ...
    print("Starting training....", file = sourceFile)
    print("Training data size:",len(model.train_dataloader()))
    trainer.fit(model)
    print("Calling test...", file = sourceFile)
    trainer.test()
...
if __name__ == '__main__':
    ...
    print("Calling main function inside trainer.py", file = sourceFile)
    main()
